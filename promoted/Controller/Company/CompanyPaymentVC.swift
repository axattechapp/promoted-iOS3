...
class CompanyPaymentVC: BaseViewController, UITextFieldDelegate{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        lblAmount.text = "Recurring monthly payment - $\(amount)"
    ...
    func GetStripeToken()
    {
        ...
        guard let token = token, error == nil else {
            ...
            if self.planId == "" {
                self.oneTimePaymentApi(token_Id: "\(token)")
    ...
    func PaymentApi(token_Id:String)
    {
        ...
        let month    = fullNameArr[0]
        let year = fullNameArr[1]
        //plan_id,card_number,exp_month,exp_year,cvc
        let params:[String:Any] = ["plan_id":"\(planId)","card_number":"\(tfCard.text!)","exp_month":"\(month)", "exp_year":"\(year)", "cvc":"\(tfCvv.text!)","source_token":"\(token_Id)"]
        print(params)
        
        ONetworkManager.sharedInstance.requestApi(withServiceName: ServiceName.companyPayment,                                                   requestMethod: .POST,
        ...
        {[weak self](result: Any?, error: Error?, errorType: ErrorType?, statusCode: Int?) in
            if errorType == .requestSuccess{
                ...
                case 200:
                    ...
                    if status == "SUCCESS"{
                        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "companySubSuccessVC") as? companySubSuccessVC
                        self?.navigationController?.pushViewController(vc!, animated: false)
    ...
    func oneTimePaymentApi(token_Id:String)
    {
        CommonUtils.showHudWithNoInteraction(show: true)
        ...
        let year = fullNameArr[1]
        //plan_id,card_number,exp_month,exp_year,cvc
        let params:[String:Any] = ["amount":"\(amount)","card_number":"\(tfCard.text!)","exp_month":"\(month)", "exp_year":"\(year)", "cvc":"\(tfCvv.text!)","source_token":"\(token_Id)"]
        print(params)
        
        
        ONetworkManager.sharedInstance.requestApi(withServiceName: ServiceName.oneTimeCompanyPayment,                                                   requestMethod: .POST,
                                                  requestParameters:params, withProgressHUD: false)
        {[weak self](result: Any?, error: Error?, errorType: ErrorType?, statusCode: Int?) in
            if errorType == .requestSuccess{
                let dictResult = kSharedInstance.getDictionary(result)  //result as! [String: Any]
