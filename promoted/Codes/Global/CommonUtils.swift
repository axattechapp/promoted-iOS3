...
//
    ...
    static func showToastForDefaultError() {
        Toast(text:"Server Error", duration: Delay.long).show()
//        let view = MessageView.viewFromNib(layout: .cardView)
//        view.configureTheme(.error)
//        view.configureDropShadow()
//        view.button?.isHidden = true
//        view.configureContent(title: kAlertTitle, body: kDefaultErrorMsg)
//        view.layoutMarginAdditions = UIEdgeInsets(top: 20, left: 10, bottom: 20, right: 10)
//        SwiftMessages.show(view: view)
    }
    ...
    static func showToastForInternetUnavailable() {
        ...
        view.configureDropShadow()
        view.button?.isHidden = true
        view.configureContent(title: kAlertTitle, body: kNoInternetMsg)
        view.layoutMarginAdditions = UIEdgeInsets(top: 20, left: 10, bottom: 20, right: 10)
        SwiftMessages.show(view: view)
    }
    ...
    static func showHudWithNoInteraction(show: Bool) {
        if show {
            ...
            SVProgressHUD.setDefaultStyle(.custom)
            SVProgressHUD.setForegroundColor(#colorLiteral(red: 0.3490196078, green: 0.4862745098, blue: 0.9254901961, alpha: 1))
            SVProgressHUD.setBackgroundColor(#colorLiteral(red: 0, green: 0, blue: 0, alpha: 0).withAlphaComponent(0))
            SVProgressHUD.show()
        } else {
            SVProgressHUD.dismiss()
    ...
    static func getAttributedString(boldString: String,
        ...
        let attrs = [NSAttributedString.Key.font : UIFont.init(name: "ProximaNova-Bold", size: fontSize)]
                 ...
                 if let urlData = NSData(contentsOf: videpUrl!){
                     ...
                     CommonUtils.saveVideoToAlbum(videoUrl) { (error) in
                         ...
                         if error == nil {
                            ...
                            Toast(text:"Video saved successfully", duration: Delay.long).show()
