...
let AlmofireApiInstanse = ONetworkManager.sharedInstance
...
public class ONetworkManager {
    ...
    //Old Instanse
    internal static let sharedInstance: ONetworkManager = {
        return ONetworkManager()
    }()
    
    var customDelegate : ONetworkManagerDelegate?
        ...
        if NetworkReachabilityManager()?.isReachable == true {
            ...
            Alamofire.upload(multipartFormData:{ (multipartFormData: MultipartFormData) in
                ...
                for dictImage in arrImages {
                    ...
                    else if validDict["image"] is [UIImage]  {
                            ...
                            if let url = URL.init(string: urlString) {
                                do {
                                    ...
                                    if String.getString(postData[kKey]) == "2" {
                                        multipartFormData.append(data, withName: String.getString(validDict["imageName"]), fileName: String.getString(NSNumber.getNSNumber(message: self.getCurrentTimeStamp()).intValue) + ".mp4", mimeType: "video/mp4")
                                    }
                                    else {
...
// MARK:- PROTOCOLs
protocol ONetworkManagerDelegate {
