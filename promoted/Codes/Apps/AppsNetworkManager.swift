...
//            print("Response: \(String(describing: data)) --- ResponseIs: \(String(describing: response)) --- ErrorIs \(String(describing: error))")
            ...
            if let data = data {
                ...
                do {
                    ...
                    switch statusCode{
                    case 200 :
                        DispatchQueue.main.async {completionClosure(response)}
                    case 401:
...
extension AppsNetworkManager {
    ...
    func showHudWithNoInteraction(show: Bool) {
        if show {
            ...
            SVProgressHUD.setForegroundColor(#colorLiteral(red: 0.2235294118, green: 0.3254901961, blue: 0.6431372549, alpha: 1))
...
struct AppsNetworkManagerConstants {
    static let baseUrl               = "http://15.184.155.113/"
    static let baseUrlForimage       = "http://15.185.178.48:3001"
